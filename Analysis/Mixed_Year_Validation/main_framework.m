...
for s_idx = 1:num_scenarios
    ...
    data_23 = load_data(base_path_23, '2023', num_days, [curr_stages{:}], curr_angles, curr_indices, ch_per_angle, num_multi);
    data_24 = load_data(base_path_24, '2024', num_days, [curr_stages{:}], curr_angles, curr_indices, ch_per_angle, num_multi);
    ...
    obj_func = @(x) wrapper_framework(x, data_store, num_varieties, ...
                    curr_stages, pinvA23, curr_stages, pinvA24, fixed_aug, ...
                    fixed_noise, fixed_scale);

    results_opt = bayesopt(obj_func, vars, 'MaxObjectiveEvaluations', 50, 'Verbose', 0, 'PlotFcn', []);

    best_p = results_opt.XAtMinObjective;
    [~, accuracy, f_cve, f_npca] = logic_framework(best_p, data_store, num_varieties, ...
                                            curr_stages, pinvA23, curr_stages, pinvA24, fixed_aug, ...
                                            fixed_noise, fixed_scale);
