function [error, accuracy, cve_val, n_pca_actual] = logic_framework(opt_vars, data_store, num_varieties, stages23, pinvA23, stages24, pinvA24, num_aug, noise_level, scale_range)
    ...
    train_idx_calib = zeros(num_varieties * 4, 1);
    for v = 1:num_varieties
        train_idx_calib((v-1)*4 + (1:4)) = (v-1)*8 + (1:4);
    end
    
    train_23_raw = data_store(train_idx_calib, :, :);
    global_min = min(min(train_23_raw, [], 1), [], 2); 
    global_max = max(max(train_23_raw, [], 1), [], 2);
    global_range = global_max - global_min;
    
    data_store_calibrated = (data_store - global_min) ./ (global_range + eps);
    ...
    if isempty(n_pca_actual)
        n_pca_actual = length(explained); 
    end
